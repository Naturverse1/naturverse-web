...
export default function NavatarCreator() {
  ...
  const handleCreateNavatar = async () => {
    ...
    try {
      ...
      setSuccess(
        `🎉 Your ${categoryData?.name.split(' ')[1]} navatar has been created successfully!`,
      );
          ...
          {categories.map((category) => (
            ...
            >
              <CardHeader className="text-center">
                <CardTitle className="text-2xl font-fredoka text-forest">{category.name}</CardTitle>
              </CardHeader>
              ...
              <CardContent className="pt-6">
                <div className="mb-4">
                  ...
                  <p className="text-forest/80 mb-4">
                    Your {categories.find((c) => c.id === selectedCategory)?.name.split(' ')[1]}{' '}
                    navatar will be created!
                  </p>
                </div>
