...
function loadCustom(): Story[] {
  try { return JSON.parse(localStorage.getItem(STORAGE_STORIES) || "[]"); } catch { return []; }
}
function saveCustom(list: Story[]) {
  try { localStorage.setItem(STORAGE_STORIES, JSON.stringify(list)); } catch {}
}
...
function saveProgress(map: Record<string, Progress>) {
  try { localStorage.setItem(STORAGE_PROGRESS, JSON.stringify(map)); } catch {}
}
...
export default function Stories() {
  ...
  const addChoice = (from: string) => setLinks(v => [...v, { from, text: "Continue", to: "start" }]);
  const saveStory = () => {
    const story: Story = {
      id: `local-${uid()}`,
      ...
      scenes: scenes.map(s => ({
        ...
        choices: links.filter(l => l.from === s.id).map((l, i) => ({ id: `${s.id}-${i}`, text: l.text, goto: l.to }))
      }))
    };
    const next = [story, ...custom];
    setCustom(next); saveCustom(next);
    setTab("library"); setActiveId(story.id);
  };
  ...
  return (
    <div>
      ...
      <div className="tabs" role="tablist" aria-label="Stories">
        ...
        <button className="tab" aria-selected={tab==="create"} onClick={()=>setTab("create")}>Create</button>
        <button className="tab" aria-selected={tab==="library"} onClick={()=>setTab("library")}>Library</button>
            ...
            <div className="list-grid">
              {stories.map(s=>(
                <button key={s.id}
                        ...
                        style={{ textAlign:"left", border: s.id===activeId ? "2px solid #111827" : undefined }}>
      ...
      {tab === "create" && (
        <section>
          ...
          <div className="form-row">
            ...
            <input className="input" placeholder="Realm (e.g., Amazonia)" value={realm} onChange={e=>setRealm(e.target.value)}/>
          ...
          <div className="list-grid">
            {scenes.map((s, idx)=>(
              <div key={s.id} className="card-mini">
                <div className="meta">id: {s.id}{idx===0 ? " (start)" : ""}</div>
                <input className="input" value={s.title} onChange={e=>{
                  const v=[...scenes]; v[idx]={...v[idx], title:e.target.value}; setScenes(v);
                }}/>
                <textarea className="textarea" rows={3} value={s.body} onChange={e=>{
                  const v=[...scenes]; v[idx]={...v[idx], body:e.target.value}; setScenes(v);
                }}/>
                <button className="btn outline" onClick={()=>addChoice(s.id)}>Add choice from this scene</button>
              </div>
            ))}
          </div>
          <div className="actions">
            <button className="btn outline" onClick={addScene}>Add scene</button>
          </div>
          ...
          <div className="list-grid">
            {links.map((l, i)=>(
              <div key={i} className="card-mini">
                ...
                <input className="input" value={l.text} onChange={e=>{
                  const v=[...links]; v[i]={...v[i], text:e.target.value}; setLinks(v);
                }}/>
                <select className="select" value={l.to} onChange={e=>{
                  const v=[...links]; v[i]={...v[i], to:e.target.value}; setLinks(v);
                }}>
                  {scenes.map(s=> <option key={s.id} value={s.id}>{s.id} – {s.title}</option>)}
                </select>
              </div>
            ))}
          </div>
          ...
          <div className="actions">
            <button className="btn" onClick={saveStory}>Save to Library</button>
          </div>
      ...
      {tab === "library" && (
        <section>
          <h2>My Library</h2>
          {custom.length === 0 ? <p className="meta">No custom stories yet.</p> : (
            <div className="list-grid">
              {custom.map(s=>(
                <div key={s.id} className="card-mini">
                  <div style={{fontSize:22}}>{s.emoji || "📖"}</div>
                  ...
                  <div className="actions" style={{marginTop:8}}>
                    <button className="btn outline" onClick={()=>navigator.clipboard.writeText(JSON.stringify(s,null,2))}>Copy JSON</button>
                    <button className="btn" onClick={()=>{ setActiveId(s.id); setTab("play"); }}>Play</button>
                  </div>
                </div>
              ))}
            </div>
