...
function saveCustom(qs: Quiz[]) {
  try {
    localStorage.setItem(LS_CUSTOM, JSON.stringify(qs));
  } catch {}
}
...
export default function Quizzes() {
  ...
  const [qs, setQs] = useState<
    { prompt: string; a: string; b: string; c: string; d: string; correct: 'a' | 'b' | 'c' | 'd' }[]
  ...
  return (
    <div className="page-wrap zones-page zones-page2">
      ...
      <div className="tabs" role="tablist" aria-label="Quizzes">
        ...
        <button className="tab" aria-selected={tab === 'party'} onClick={() => setTab('party')}>
          Party
        </button>
        <button className="tab" aria-selected={tab === 'builder'} onClick={() => setTab('builder')}>
          Builder
        </button>
      ...
      {tab === 'play' && (
        <section className="creator-grid">
            ...
            <div className="list-grid">
              {quizzes.map((q) => (
                <button
                  ...
                  style={{
                    textAlign: 'left',
                    border: q.id === activeId ? '2px solid var(--nv-text)' : undefined,
                  }}
                >
                  <div style={{ fontSize: 22 }}>{q.emoji || '🧠'}</div>
                  <div style={{ fontWeight: 700 }}>{q.title}</div>
                  <div className="meta">
                    {q.mode === 'classic' ? 'Multiple Choice' : 'Jeopardy Board'}
                  </div>
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            {active.mode === 'jeopardy' && <JeopardyBoard quiz={active} />}
          </div>
        </section>
      )}
      ...
      {tab === 'party' && (
        <section>
          ...
          <p className="meta">
            Use the Jeopardy board on a shared screen. Tap a tile, read the clue, award points with
            “Correct”.
          </p>
      ...
      {tab === 'builder' && (
        <section>
          ...
          <div className="actions">
            <button className="btn outline" onClick={addQ}>
              Add question
            </button>
